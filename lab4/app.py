...
def create():
    if request.method == 'POST':
        ...
        insert_query = '''INSERT INTO users (login, last_name, first_name, password_hash)
...
def change():
    if request.method == "POST":
        ...
        check_password_query = 'SELECT * FROM users WHERE id = %s AND password_hash = SHA2(%s, 256)'
        ...
        try:
            with db.connection().cursor(named_tuple=True) as cursor:
                ...
                if not user:
                    ...
                    update_password_query = 'UPDATE users SET password_hash = SHA2(%s, 256) WHERE id = %s'
